...
//		rangeStatus = sensors.readRang
/*
 * SenseInfrared.cpp
 *
 *  Created on: 19.11.2017
 *      Author: TobiZ (modified for own code by Felix Hessinger)
 */
...
void SenseInfrared::run()
{
	...
	while(1)
	{
		...
		data.angle = angle;
//		PRINTF("\nIR_ANGLE: %f",angle);
//		PRINTF("\nIR_DISTANCE: %f",distance);
//		PRINTF("\nIR_RANGE1: %f",data.range1);
//		PRINTF("\nIR_RANGE2: %f",data.range2);

		IRDataTopic.publish(data);
