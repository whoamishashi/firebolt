...
/////////////////////PID Values for Control of Reactionwheel/////////////
...
////Variables for Speed Sensor of the ReactionWheel
	//speeds
...
void ControlRW::run() {
	init();
	now = 0;
	suspendCallerUntil(WAITINGTIME_UNTIL_START*SECONDS);	//Ensure that controller doesn't start before calibration and actuator start
	...
	while(1){
		...
		measured = (rpm*1000/MAX_RPM_RW_POSSIBLE);  // divided by maximum possible RPM with specific H-Bridge times 1000 for write input (range 0 to 1000)(0to100%)
